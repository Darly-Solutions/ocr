      ...
      const modelDatas = await Promise.all(
        lineImages.map(async (lineImage, index) => {
          try {
            ...
            if (!image) {
              console.error(`Failed to resize line image ${index}`);
              return null;
            }
            ...
            console.error(`Failed to process line image ${index}:`, error);
            return null;
          }
        }),
      );

      // Filter out any failed model data conversions
      const validModelDatas = modelDatas.filter(
        (data): data is NonNullable<typeof data> => data !== null,
      );
      if (validModelDatas.length === 0) {
        return [];
      }

      const allLines: Line[] = [];
      for (const modelData of validModelDatas) {
        try {
