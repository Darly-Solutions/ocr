...
// Cluster object: average Y position and associated lines
interface Cluster {
  y: number;
  lines: LineImage[];
}

interface GroupSharpness {
  sharpness: number;
  line: LineImage;
  groupIndex: number;
}
...
function filterBySharpestInGroups(clusters: Cluster[], minSharpnessRatio = 0.7) {
  const groupSharpnesses: GroupSharpness[] = [];
...
function clusterLinesByY(lines: LineImage[], yThreshold = Y_THRESHOLD): Cluster[] {
  const clusters = [];
