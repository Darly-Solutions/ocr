...
export class Detection extends ModelBase {
  ...
  async run(
    ...
    try {
      ...
      if (!image) {
        console.error('Failed to open image:', path);
        return [];
      }
      ...
      // Convert image data to model data
      ...
      if (!outputImage) {
        console.error('Failed to convert model output to image');
        return [];
      }
  ...
  filterText(lineImages: LineImage[], frameWidth: number, frameHeight: number) {
    return lineImages.filter((lineImage) => {
      ...
      return (
        isTextAreaValid(box, frameWidth, frameHeight) &&
        !isTextSkewed(box) &&
...
function isTextAreaValid(
  ...
  frameWidth: number,
  frameHeight: number,
  MIN_TEXT_AREA_PERCENT = 0.0037,
  MAX_TEXT_AREA_PERCENT = 0.9,
) {
  ...
  if (textAreaPercent < MIN_TEXT_AREA_PERCENT) {
    ...
    return (
      y1 > 0.8 * frameHeight &&
      Math.abs((x1 + x2) / 2 - frameWidth / 2) < 0.02 * frameWidth
    );
  }
...
function isSubtitleOrHeading(
  textBox: any,
  frameWidth: number,
  frameHeight: number,
  horizontalTolerance = 0.07,
  verticalTolerance = 0.15,
) {
  ...
  if (
    textCenterY < frameHeight * 0.2 ||
    textCenterY > frameHeight * 0.8 ||
    ((textCenterY > frameHeight * 0.45 || textCenterY < frameHeight * 0.55) &&
      centerX - toleranceX <= textCenterX &&
      textCenterX <= centerX + toleranceX)
  ) {
    return true;
  }
  return false;
