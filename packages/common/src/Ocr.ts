import type { Line, LineImage, ModelCreateOptions } from '#common/types';
...
export class Ocr {
  ...
  async detect(image: string, options = {}): Promise<LineImage[]> {
    // run ocr pipeline for each YOLO output
    return await this.#detection.run(image, options);
  }

  async recognize(lineImages: LineImage[], options = {}): Promise<Line[]> {
    return await this.#recognition.run(lineImages, options);
  }
}
